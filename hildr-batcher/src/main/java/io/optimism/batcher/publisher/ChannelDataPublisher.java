...
public class ChannelDataPublisher implements Closeable {
    ...
    public ChannelDataPublisher(
        ...
        this.txManager = new RawTransactionManager(
                l1Client, credentials, config.l1chainId().longValue());
    ...
    private void sendTx(final Frame txData) {
        ...
        long intrinsicGas = GasCalculator.intrinsicGasWithoutAccessList(txData.txBytes(), false, true, true, false);
    ...
    private EthGetTransactionReceipt getTxReceipt(final String txHash) {
        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
            var receiptFuture =
                    scope.fork(() -> l1Client.ethGetTransactionReceipt(txHash).send());
            scope.join();
            scope.throwIfFailed();
    ...
    private L1BlockRef getL1HeadBlockRef() {
        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
            StructuredTaskScope.Subtask<EthBlock.Block> headBlockFuture = scope.fork(
                    TracerTaskWrapper.wrap(() -> l1Client.ethGetBlockByNumber(DefaultBlockParameterName.LATEST, false)
                            .send()
                            .getBlock()));
            scope.join();
            scope.throwIfFailed();
    ...
    private synchronized BigInteger nextNonce() {
        ...
        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
            var countFuture = scope.fork(TracerTaskWrapper.wrap(() -> {
                var countReq = this.l1Client.ethGetTransactionCount(this.fromAddress, DefaultBlockParameterName.LATEST);
                return countReq.send();
            }));
            scope.join();
            scope.throwIfFailed();
            txCount = countFuture.get();
    ...
    private BigInteger getMaxPriorityFeePerGas() {
        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
            var gasFuture = scope.fork(TracerTaskWrapper.wrap(() -> {
                var countReq = this.l1Client.ethMaxPriorityFeePerGas();
                return countReq.send();
            }));
            scope.join();
            scope.throwIfFailed();
            return gasFuture.get().getMaxPriorityFeePerGas();
        } catch (InterruptedException | ExecutionException e) {
            Thread.currentThread().interrupt();
    ...
    private BigInteger getBaseFee() {
        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
            var gasFuture = scope.fork(TracerTaskWrapper.wrap(() -> {
                var headReq = this.l1Client.ethGetBlockByNumber(DefaultBlockParameterName.LATEST, false);
                return headReq.send();
            }));
            scope.join();
            scope.throwIfFailed();
            return gasFuture.get().getBlock().getBaseFeePerGas();
        } catch (InterruptedException | ExecutionException e) {
            Thread.currentThread().interrupt();
    ...
    private BigInteger getEstimateGas(final String to, final String data) {
        ...
        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
            var gasFuture = scope.fork(TracerTaskWrapper.wrap(() -> {
                final Transaction txParam = new Transaction(this.fromAddress, null, null, null, to, null, data);
                return this.l1Client.ethEstimateGas(txParam).send();
            }));
            scope.join();
            scope.throwIfFailed();
            return gasFuture.get().getAmountUsed();
        } catch (InterruptedException | ExecutionException e) {
            Thread.currentThread().interrupt();
            throw new Web3jCallException("get tx count failed", e);
        }
    ...
    private String signAndSend(final RawTransaction rawTx) {
        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
            ...
            scope.join();
            scope.throwIfFailed();
            return txResultFuture.get().getTransactionHash();
        } catch (InterruptedException | ExecutionException e) {
            Thread.currentThread().interrupt();
