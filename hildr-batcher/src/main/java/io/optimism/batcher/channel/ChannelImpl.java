...
public class ChannelImpl implements Channel {
    ...
    public ChannelImpl(ChannelConfig chConfig, Compressor compressor) {
        ...
        this.seqWindowTimeout = BigInteger.valueOf(chConfig.seqWindowSize() - chConfig.subSafetyMargin());
    ...
    public void txFailed(Frame tx) {
        ...
        if (!this.pendingTxs.containsKey(code)) {
            LOGGER.warn("unkown tx marked as failed: chId :{}; frameNum: {}", tx.channelId(), tx.frameNumber());
            return;
        }
        LOGGER.trace("marked transaction as failed: chId :{}; frameNum: {}", tx.channelId(), tx.frameNumber());
        this.pushFrame(tx);
    ...
    private Tuple2<L1BlockInfo, Batch> blockToBatch(EthBlock.Block block) {
        ...
        final L1BlockInfo l1Info = L1BlockInfo.from(Numeric.hexStringToByteArray(depositTxObj.getInput()));
    ...
    private int addBatch(Batch batch) {
        ...
        if ((this.rlpLength.get() + encode.length) > MAX_RLP_BYTES_PER_CHANNEL) {
            throw new ChannelFullException(String.format(
