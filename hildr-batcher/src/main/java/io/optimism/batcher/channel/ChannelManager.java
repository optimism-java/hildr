...
public class ChannelManager {
    ...
    public void txFailed(final Frame tx) {
        ...
        LOGGER.info("Channel has no submitted transactions, clearing for shutdown: chId: {}", tx.channelId());
    ...
    private void pushBlocks(final Channel lastChannel) {
        ...
        try {
            ...
            if (!(e instanceof ChannelFullException)) {
                LOGGER.error(
                        "adding block[{}] to channel failed",
                        this.blocks.get(blocksAdded).getNumber(),
                        e);
            }
        }
    ...
    private void updateChannelTimeout(BlockId l1Head) {
        this.latestChannel.updateTimeout(l1Head.number().add(BigInteger.valueOf(chConfig.maxChannelDuration())));
    }
}
