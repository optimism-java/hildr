...
public class TxPrometheusMetrics implements TxMetrics {
  ...
  static {
    ...
    DESC_MAP.put(
        "tx_publish_error_count", "Count of publish errors. Labels are sanitized error strings.");
    DESC_MAP.put(
        "rpc_error_count", "Temporary: Count of RPC errors (like timeouts) that have occurred.");
  }
  ...
  public void txConfirmed(TransactionReceipt receipt) {
    ...
    BigDecimal fee =
        Convert.fromWei(effectiveGasPrice.divide(gasUsed, 2, RoundingMode.UP), Convert.Unit.GWEI);
    this.metricsSupplier.getOrCreateGaugeDecimal("tx_fee_gwei", EMPTY_TAGS).getAndSet(fee);
    this.metricsSupplier
        .getOrCreateCounter("tx_fee_gwei_total", EMPTY_TAGS)
        .increment(fee.doubleValue());
    this.metricsSupplier
  ...
  public void recordTxConfirmationLatency(long latency) {
    this.metricsSupplier.getOrCreateGauge("tx_confirmed_latency_ms", EMPTY_TAGS).getAndSet(latency);
  }
  ...
  public void txPublished(String reason) {
    if (StringUtils.isEmpty(reason)) {
      ...
      this.metricsSupplier.getOrCreateCounter("tx_publish_error_count", tags).increment(1);
    } else {
