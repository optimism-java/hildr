...
public class MetricsSupplier {
   ...
   * @return gauge value.
   */
  public AtomicReference<BigDecimal> getOrCreateGaugeDecimal(
      String name, final Map<String, String> tagKeyValue) {
    final String label = name + tagKeyValue.toString();
    var value =
        this.numberMap.computeIfAbsent(
            label,
            key -> {
              var guage = new AtomicReference<BigDecimal>();
              Gauge.Builder<AtomicReference<BigDecimal>> gaugeBuilder =
                  Gauge.builder(withPrefix(name), guage, ref -> ref.get().doubleValue())
    ...
    return (AtomicLong)
        this.numberMap.computeIfAbsent(
            ...
            key -> {
              ...
              Gauge.Builder<AtomicLong> gaugeBuilder =
                  Gauge.builder(withPrefix(name), guage, AtomicLong::doubleValue)
                      .description(descMap.get(name));
              if (tagKeyValue.size() > 0) {
                final List<Tag> tags = toList(tagKeyValue);
                gaugeBuilder.tags(Tags.of(tags));
  ...
  public DistributionSummary getOrCreateHistogram(
    ...
    final String label = name + tagKeyValue.toString();
    return this.histogramMap.computeIfAbsent(
        label,
        key -> {
          Arrays.sort(buckets);
          DistributionSummary.Builder builder =
              DistributionSummary.builder(withPrefix(name))
                  .description(descMap.get(name))
  ...
  public Counter getOrCreateCounter(String name, final Map<String, String> tagKeyValue) {
    final String label = name + tagKeyValue.toString();
    return this.counterMap.computeIfAbsent(
        label,
        key -> {
          Counter.Builder counterBuilder =
              Counter.builder(withPrefix(name)).description(descMap.get(name));
          if (tagKeyValue.size() > 0) {
            final List<Tag> tags = toList(tagKeyValue);
  ...
  public EventMeter getOrCreateEventMeter(String name, final Map<String, String> tagKeyValue) {
    ...
    return this.eventMap.computeIfAbsent(
        eventLabel,
        label -> {
          Counter.Builder counterBuilder =
              Counter.builder(withPrefix(name + "_total"))
                  .description(String.format("Count of %s events", name));

          final AtomicLong guage = new AtomicLong();
          Gauge.Builder<AtomicLong> gaugeBuilder =
              Gauge.builder(
                      withPrefix(String.format("last_%s_unix", name)),
                      guage,
