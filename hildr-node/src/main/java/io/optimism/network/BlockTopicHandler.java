            ...
            return SSZ.decode(data, sszReader -> {
                ...
                if (extraDataOffset != EXECUTION_PAYLOAD_FIXED_PART) {
                    throw new IllegalArgumentException(String.format(
                            "unexpected extra data offset: %d <> %d", extraDataOffset, EXECUTION_PAYLOAD_FIXED_PART));
                }
                ...
                if (sszReader.isComplete()) {
                    ...
                    transactions = SSZ.decode(transactionsBytes, txsSSZReader -> {
                        int transactionsBytesSize = transactionsBytes.size();
                        if (transactionsBytesSize < 4) {
                            throw new IllegalArgumentException(String.format(
                                    "not enough scope to read first tx offset: %d", transactionsBytesSize));
                        }

                        long firstTxOffset = txsSSZReader.readUInt32();
                        if (firstTxOffset % 4 != 0) {
                            throw new IllegalArgumentException(String.format(
                                    "invalid first tx offset: %d, not a multiple of offset size", firstTxOffset));
                        }
                        ...
                        if (txCount > MAX_TRANSACTIONS_PER_PAYLOAD) {
                            throw new IllegalArgumentException(String.format(
                                    "too many transactions: %d > %d", txCount, MAX_TRANSACTIONS_PER_PAYLOAD));
                        }
