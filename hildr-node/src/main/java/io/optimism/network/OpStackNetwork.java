...
public class OpStackNetwork {
  ...
  private static final List<String> BOOTNODES =
      List.of(
  ...
  public OpStackNetwork(Config.ChainConfig config, MessagePassingQueue<ExecutionPayload> unsafeBlockQueue) {
    ...
    final DiscoveryConfig discoveryConfig =
        DiscoveryConfig.builder().listenUdpPortDefault(9876).bootnodes(BOOTNODES).build();
    ...
    this.topicHandler =
        new BlockTopicHandler(
            ...
            asyncRunner,
                chainId,
            config.systemConfig().unsafeBlockSigner(),unsafeBlockQueue);
    ...
    final PeerSelectionStrategy peerSelectionStrategy =
        new SimplePeerSelectionStrategy(targetPeerRange);
    this.p2pNetwork =
        builder
            .kvStore(kvStore)
            .asyncRunner(asyncRunner)
            ...
            (Consumer<Object>)
                o ->
                    p2pNetwork.subscribe(
                        ((BlockTopicHandler) topicHandler).getTopic(), topicHandler))
        .finish(
            error -> {
              if (error != null) {
